...
def test_sanitize_session_id():
    ...
    assert (
        nwb2bids.sanitization.sanitize_session_id(
            session_id="Session 12 subject 5", sanitization_level=sanitization_level
        )
    ...
    assert (
        nwb2bids.sanitization.sanitize_session_id(
            session_id="2025-02-03T01:08:12.1236", sanitization_level=sanitization_level
        )
        == "2025+02+03T01+08+12+1236"
    )

    assert (
        nwb2bids.sanitization.sanitize_session_id(session_id="12_02_2025", sanitization_level=sanitization_level)
        == "12+02+2025"
    )
